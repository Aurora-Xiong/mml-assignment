...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h = np.dot(prev_h, Wh) + np.dot(x, Wx) + b
    next_h = np.tanh(h)
    cache = (x, prev_h, Wx, Wh, b, h)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i1, f1, o1, c1 = np.split(x.dot(Wx) + prev_h.dot(Wh) + b, 4, axis=1)
    i2, f2, o2, c2 = sigmoid(i1), sigmoid(f1), sigmoid(o1), np.tanh(c1)
    next_c = f2 * prev_c + i2 * c2
    next_h = o2 * np.tanh(next_c)
    cache = (x, prev_h, prev_c, Wx, Wh, b, i1, f1, o1, c1, next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, i1, f1, o1, c1, next_c= cache
    do = dnext_h * np.tanh(next_c) * sigmoid(o1) * sigmoid(-o1)
    dnext_c += dnext_h * sigmoid(o1) * (1 - np.tanh(next_c)**2)
    dc = dnext_c * sigmoid(i1) * (1 - np.tanh(c1)**2)
    di = dnext_c * np.tanh(c1) * sigmoid(i1) * sigmoid(-i1)
    df = dnext_c * prev_c * sigmoid(f1) * sigmoid(-f1)
    dprev_c = dnext_c * sigmoid(f1)
    dall = np.concatenate((di, df, do, dc), axis=1)
    dx = dall.dot(Wx.T)
