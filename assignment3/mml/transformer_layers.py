...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        tmp = torch.arange(max_len).unsqueeze(1) / torch.pow(10000, torch.arange(0, embed_dim, 2) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = torch.stack(self.query(query).split(self.head_dim, dim=-1), dim=1)
        key = torch.stack(self.key(key).split(self.head_dim, dim=-1), dim=1)
        value = torch.stack(self.value(value).split(self.head_dim, dim=-1), dim=1)
        ...
        output = torch.matmul(attn, value).transpose(1, 2).reshape(N, S, E)
