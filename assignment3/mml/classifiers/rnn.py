...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            pred, cache_temporal_affine = temporal_affine_forward(h, W_vocab, b_vocab)
            loss, dpred = temporal_softmax_loss(pred, captions_out, mask)
            dh, dW_vocab, db_vocab = temporal_affine_backward(dpred, cache_temporal_affine)
            ...
            loss, dpred = temporal_softmax_loss(pred, captions_out, mask)
            dh, dW_vocab, db_vocab = temporal_affine_backward(dpred, cache_temporal_affine)
            dword_vectors, dh0, dWx, dWh, db = lstm_backward(dh, cache_lstm)
            dW_embed = word_embedding_backward(dword_vectors, cache_embed)
            dW_proj = features.T.dot(dh0)
            db_proj = np.sum(dh0, axis=0)
    ...
    def sample(self, features, max_length=30):
        ...
        h0 = features.dot(W_proj) + b_proj
        prev_h = h0
        x = word_embedding_forward(np.ones((N, 1), dtype=np.int32) * self._start, W_embed)[0][:, 0, :]
        ...
        if self.cell_type == 'rnn':
            for t in range(max_length):
                ...
                stopped = stopped | (captions[:, t] == self._end)
                captions[stopped, t] = self._null
                prev_h = next_h
                x = word_embedding_forward(captions[:, t].reshape(-1, 1), W_embed)[0][:, 0, :]
        elif self.cell_type == 'lstm':
